...
def migrate_legacy_quadratic_color(color: dict) -> dict | None:
    """Convert {background, color_map} -> {background_color, color_maps}.

    Also normalizes earlier Phase-3 outputs that used `gradients` / the
    `flat_color` spelling into the post-review `color_maps` /
    `background_color` shape.
    """
    if not isinstance(color, dict):
        return None
    if "background_color" in color and "color_maps" in color:
        return None  # already migrated to the post-review shape
    if "flat_color" in color and "color_maps" in color:
        return {
            "background_color": color["flat_color"],
...
def migrate_ddp(params: dict) -> bool:
    ...
    Also normalizes earlier Phase-3 outputs (`gradients`, `flat_color`)
    into the post-review `color_maps` / `background_color` shape.
    """
...
def migrate_newton(params: dict) -> bool:
    ...
    `ColorPalette`. Also normalizes earlier Phase-3 outputs (`gradients`,
    `flat_color`) into the post-review `color_maps` / `background_color`
    shape. Returns True iff a change was made."""
    color = params.get("color")
    changed = False
    if isinstance(color, dict):
        already_post_review = "background_color" in color and "color_maps" in color
        if not already_post_review:
            if "flat_color" in color and "color_maps" in color:
                ...
                changed = True
            elif "flat_color" in color and "gradients" in color:
                params["color"] = {
                    "background_color": color["flat_color"],
                    "color_maps": color["gradients"],
                }
                ...
                params["color"] = {
                    "background_color": color["cyclic_attractor"],
                    "color_maps": color["color_maps"],
                }
                changed = True
